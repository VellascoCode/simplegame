...
const STOP_DISTANCE = MAP_TILE_SIZE;
...
const HERO_ATTACK_COOLDOWN = 400;
const COMBAT_ZONE_RADIUS = 1;
const AGGRO_COOLDOWN_MS = 1000;
const PUSH_STEP = MAP_TILE_SIZE;
const MAX_PUSH_RANGE = MAP_TILE_SIZE;
const FACTION_SKIRMISH_RANGE = MAP_TILE_SIZE * 5;
const WORLD_CLEANUP_INTERVAL = 60 * 60 * 1000;
...
function isInCombatZone(center, target) {
    return Math.abs(center.x - target.x) <= COMBAT_ZONE_RADIUS && Math.abs(center.y - target.y) <= COMBAT_ZONE_RADIUS;
}
function CityPhaser({ ownerId, characterId, characterName, characterLevel, characterXp, initialPosition, onPositionChange, onCombatEvent, onStatsChange, onGoldChange, onBestiaryUpdate, warState, soundEnabled = true, onReady }) {
    ...
    const warStateRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useRef"])(warState ?? null);
    ...
    const soundEnabledRef = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useRef"])(soundEnabled);
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        ...
        (async ()=>{
            ...
            const detailLookup = new Map(detailTiles.map((tile)=>[
                    tile.id,
                    tile
                ]));
            ...
            class CityScene extends Phaser.Scene {
                ...
                preload() {
                    ...
                    Object.entries(LOOT_ICON_PATHS).forEach(([itemId, path])=>{
                ...
                create() {
                    ...
                    this.time.addEvent({
                        delay: 110,
                        loop: true,
                        callback: ()=>this.advanceNPCs(0.11)
                    });
                    this.spawnLancers(LANCER_COUNT);
                    this.setupInputs();
                    this.scheduleWorldCleanup();
                ...
                renderDetails() {
                    mapReference.detail.forEach((row, rowIndex)=>{
                        row.forEach((cell, columnIndex)=>{
                            if (cell <= 0) return;
                            const detailDef = detailLookup.get(cell) ?? detailTiles[cell - 1];
                            const textureKey = detailDef ? `detail-${detailDef.id}` : `detail-${cell}`;
                ...
                renderBuildings() {
                    mapReference.buildings.forEach((row, rowIndex)=>{
                        row.forEach((cell, columnIndex)=>{
                            if (cell <= 0) return;
                            const def = buildingLookup.get(cell) ?? buildingTiles[cell - 1];
                            if (!def) return;
                ...
                spawnNPCs() {
                    ...
                    for(let i = 0; i < 4; i++){
                        const faction = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$factions$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getRandomFaction"])();
                        ...
                        const npc = {
                            ...
                            faction,
                            lastLabelX: sprite.x,
                            lastLabelY: sprite.y - sprite.displayHeight - 4
                        };
                ...
                scheduleWorldCleanup() {
                    ...
                    this.cleanupTimer = this.time.addEvent({
                        delay: WORLD_CLEANUP_INTERVAL,
                        loop: true,
                        callback: ()=>this.cleanWorldState()
                    });
                }
                cleanWorldState() {
                    const now = this.time.now;
                    this.lancers = this.lancers.filter((lancer)=>{
                    ...
                    this.groundDrops = this.groundDrops.filter((drop)=>{
                        if (drop.expiresAt > now) return true;
                ...
                spawnLancer(index = 0) {
                    ...
                    const lancer = {
                        ...
                        wanderTarget: this.findWalkablePoint(),
                        nextAggroAllowed: 0,
                        lastStateChange: this.time.now,
                        lastPushAt: 0
                    };
                ...
                canMoveTo(x, y) {
                    ...
                    return !this.collisionZones.some((zone)=>Phaser.Geom.Intersects.CircleToRectangle(circle, zone));
                }
                advanceNPCs(deltaSeconds = 0.12) {
                    const speed = 90;
                    ...
                    this.npcs.forEach((npc)=>{
                        ...
                        const proposedX = npc.sprite.x + dir.x * travel;
                        const proposedY = npc.sprite.y + dir.y * travel;
                ...
                setLancerState(lancer, state) {
                    if (lancer.state === state) return;
                    lancer.state = state;
                    lancer.lastStateChange = this.time.now;
                    if (state === NPC_STATES.WANDER) {
                        lancer.wanderTarget = this.findWalkablePoint();
                    }
                }
                performWanderStep(lancer, delta) {
                    const speed = 70;
                    const dir = new Phaser.Math.Vector2(lancer.wanderTarget.x - lancer.sprite.x, lancer.wanderTarget.y - lancer.sprite.y);
                    const distance = dir.length();
                    if (distance < 6) {
                        lancer.wanderTarget = this.findWalkablePoint();
                        lancer.sprite.play(HERO_IDLE_KEY, true);
                        return;
                    }
                    dir.normalize();
                    const travel = Math.min(distance, speed * (delta / 1000));
                    const nextX = lancer.sprite.x + dir.x * travel;
                    const nextY = lancer.sprite.y + dir.y * travel;
                ...
                chaseTarget(lancer, targetX, targetY, delta) {
                    const dir = new Phaser.Math.Vector2(targetX - lancer.sprite.x, targetY - lancer.sprite.y);
                    const distance = dir.length();
                    ...
                    const travel = Math.min(distance, LANCER_SPEED * (delta / 1000));
                    const nextX = lancer.sprite.x + dir.x * travel;
                    const nextY = lancer.sprite.y + dir.y * travel;
                    if (this.canMoveTo(nextX, nextY)) {
                        lancer.sprite.x = nextX;
                ...
                resolveLancerCollisions(lancers) {
                    for(let i = 0; i < lancers.length; i++){
                        for(let j = i + 1; j < lancers.length; j++){
                            const a = lancers[i];
                            const b = lancers[j];
                            const distance = Phaser.Math.Distance.Between(a.sprite.x, a.sprite.y, b.sprite.x, b.sprite.y);
                            if (distance > 0 && distance < MAX_PUSH_RANGE) {
                                const angle = Math.atan2(a.sprite.y - b.sprite.y, a.sprite.x - b.sprite.x);
                                const offsetX = Math.cos(angle) * PUSH_STEP;
                                const offsetY = Math.sin(angle) * PUSH_STEP;
                                this.pushLancer(a, offsetX, offsetY);
                                this.pushLancer(b, -offsetX, -offsetY);
                            }
                        }
                    }
                }
                pushLancer(lancer, dx, dy) {
                    const nextX = lancer.sprite.x + dx;
                    const nextY = lancer.sprite.y + dy;
                    if (!this.canMoveTo(nextX, nextY)) return;
                    lancer.sprite.x = nextX;
                    lancer.sprite.y = nextY;
                }
                updateLancers(delta) {
                    if (!this.player) return;
                    const warInfo = warStateRef.current;
                    ...
                    activeLancers.forEach((lancer)=>{
                        const lancerTile = worldToTile(lancer.sprite.x, lancer.sprite.y);
                        const inZone = isInCombatZone(playerTile, lancerTile);
                        if (!inZone) {
                            this.setLancerState(lancer, NPC_STATES.WANDER);
                        } else if (now >= lancer.nextAggroAllowed && lancer.state !== NPC_STATES.ATTACK) {
                            this.setLancerState(lancer, NPC_STATES.AGGRO);
                            lancer.nextAggroAllowed = now + AGGRO_COOLDOWN_MS;
                        }
                        const canTargetPlayer = inZone && allowed.has(lancer);
                        const enemyTarget = this.findEnemyLancerTarget(lancer, activeLancers, warInfo, playerTile);
                        this.stepLancerState(lancer, delta, {
                            canTargetPlayer,
                            enemyTarget
                        });
                        this.updateLancerLabel(lancer);
                    });
                    this.resolveLancerCollisions(activeLancers);
                }
                stepLancerState(lancer, delta, context) {
                    switch(lancer.state){
                        case NPC_STATES.IDLE:
                            this.setLancerState(lancer, NPC_STATES.WANDER);
                            break;
                        case NPC_STATES.WANDER:
                            this.performWanderStep(lancer, delta);
                            break;
                        case NPC_STATES.AGGRO:
                            {
                                let engaged = false;
                                if (context.enemyTarget) {
                                    engaged = this.chaseTarget(lancer, context.enemyTarget.sprite.x, context.enemyTarget.sprite.y, delta);
                                    if (engaged) {
                                        this.setLancerState(lancer, NPC_STATES.ATTACK);
                                        this.handleFactionSkirmish(lancer, context.enemyTarget);
                                        this.setLancerState(lancer, NPC_STATES.WANDER);
                                    }
                                } else if (context.canTargetPlayer) {
                                    engaged = this.chaseTarget(lancer, this.player.x, this.player.y, delta);
                                    if (engaged) {
                                        this.setLancerState(lancer, NPC_STATES.ATTACK);
                ...
                findEnemyLancerTarget(lancer, lancers, warInfo, playerTile) {
                    if (!warInfo) return null;
                    const enemies = lancers.filter((candidate)=>candidate !== lancer && (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$factions$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["isOpposingFaction"])(candidate.faction, lancer.faction, warInfo));
                    if (!enemies.length) return null;
                    let best = null;
                    let bestDistance = Number.POSITIVE_INFINITY;
                    enemies.forEach((enemy)=>{
                        const enemyTile = worldToTile(enemy.sprite.x, enemy.sprite.y);
                        const nearZone = isInCombatZone(playerTile, enemyTile);
                        const d = Phaser.Math.Distance.Between(enemy.sprite.x, enemy.sprite.y, lancer.sprite.x, lancer.sprite.y);
                        if (!nearZone && d > FACTION_SKIRMISH_RANGE) {
                            return;
                        }
                        if (d < bestDistance) {
                            best = enemy;
                ...
                handleFactionSkirmish(attacker, defender) {
                    const now = this.time.now;
                    if (now - attacker.lastAttack < LANCER_COOLDOWN) return;
                    attacker.lastAttack = now;
                    const damage = Phaser.Math.Between(2, 4);
                    defender.hp -= damage;
                    this.maybeChant(attacker.faction, attacker.sprite.x, attacker.sprite.y - attacker.sprite.displayHeight);
                    this.showFloatingText(defender.sprite.x, defender.sprite.y - defender.sprite.displayHeight - 8, `-${damage}`, "#ffdca9");
                    if (defender.hp <= 0) {
                        this.handleLancerDeath(defender);
                    }
                }
                playerAttack() {
                    if (!this.player) return;
                    const now = this.time.now;
                    if (now - this.lastHeroAttack < HERO_ATTACK_COOLDOWN) return;
                    this.lastHeroAttack = now;
                    this.triggerHeroAttack();
                    const offsetX = this.playerFacing < 0 ? -MAP_TILE_SIZE / 2 : MAP_TILE_SIZE / 2;
                    const hitbox = new Phaser.Geom.Rectangle(this.player.x + offsetX - 28, this.player.y - this.player.displayHeight - 12, 56, this.player.displayHeight + 24);
                    const playerTile = worldToTile(this.player.x, this.player.y);
                    const targets = this.lancers.filter((lancer)=>{
                        if (!lancer.sprite.active) return false;
                        const lancerTile = worldToTile(lancer.sprite.x, lancer.sprite.y);
                        if (!isInCombatZone(playerTile, lancerTile)) return false;
                        return Phaser.Geom.Rectangle.Contains(hitbox, lancer.sprite.x, lancer.sprite.y - lancer.sprite.displayHeight / 2);
                    });
                    targets.forEach((lancer)=>this.applyHeroStrike(lancer));
                }
                applyHeroStrike(lancer) {
                    ...
                    this.emitCombatLog({
                        message: `${this.heroName} atingiu ${lancer.label.text} por ${heroDamage} HP`,
                        tone: "damage"
                    });
                    playToneRef(520);
                    lancer.hp -= heroDamage;
                    lancer.lastAttack = this.time.now;
                    this.setLancerState(lancer, NPC_STATES.AGGRO);
                    if (lancer.hp <= 0) {
                ...
                handleCombat(lancer) {
                    ...
                    this.maybeChant(lancer.faction, lancer.sprite.x, lancer.sprite.y - lancer.sprite.displayHeight);
                    const heroDamage = Phaser.Math.Between(HERO_DAMAGE_MIN, HERO_DAMAGE_MAX);
                ...
                handleLancerDeath(lancer) {
                    ...
                    void (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$clientApi$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["postJSON"])("/api/factions/war", {
                        faction: lancer.faction
                    }).catch(()=>undefined);
                    ...
                    this.tweens.add({
                        ...
                        onComplete: ()=>{
                            ...
                            this.time.delayedCall(LANCER_RESPAWN_DELAY, ()=>this.spawnLancer(Phaser.Math.Between(0, 1000)));
                        }
                    });
                ...
                spawnSkull(x, y) {
                    ...
                    this.time.delayedCall(SKULL_LIFETIME, ()=>this.fadeSkull(skull));
                }
                fadeSkull(skull) {
                    if (!skull || !skull.active) return;
                    this.skulls = this.skulls.filter((entry)=>entry !== skull);
                    this.tweens.add({
                        targets: skull,
                        alpha: 0,
                        duration: 350,
                ...
                spawnGroundItem(drop, quantity, x, y, durationMs = GROUND_DROP_LIFETIME) {
                    ...
                    const existing = this.groundDrops.find((record)=>record.itemId === drop.id && Phaser.Math.Distance.Between(record.x, record.y, x, y) <= DROP_ACCUMULATION_RANGE);
                    if (existing) {
                        existing.amount += quantity;
                        existing.label.setText(`x${existing.amount}`);
                        existing.expiresAt = this.time.now + durationMs;
                        this.time.delayedCall(durationMs, ()=>this.fadeGroundDrop(existing));
                        return;
                    }
                    ...
                    this.time.delayedCall(durationMs, ()=>this.fadeGroundDrop(record));
                }
                fadeGroundDrop(record) {
                    this.groundDrops = this.groundDrops.filter((entry)=>entry !== record);
                ...
                async awardGold(amount) {
                    ...
                    try {
                        const { gold } = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$clientApi$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["postJSON"])("/api/character/gold", {
                            ownerId: this.playerOwnerId,
                ...
                async recordBestiaryKill(monsterId) {
                    if (!this.playerOwnerId || !this.playerCharacterId) return;
                    try {
                        const table = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$clientApi$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["postJSON"])("/api/bestiary/update", {
                            ownerId: this.playerOwnerId,
                            characterId: this.playerCharacterId,
                            monsterId
                        });
                ...
                maybeChant(faction, x, y) {
                    const chants = (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$factions$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["getFactionChants"])(faction);
                    if (!chants.length || Math.random() > 0.18) return;
                    const message = chants[Phaser.Math.Between(0, chants.length - 1)];
                    ...
                    }).setOrigin(0.5, 1).setDepth(2000).setAlpha(0);
                    this.tweens.add({
                        targets: text,
                        alpha: 1,
                        duration: 120,
                ...
                async awardXp(amount) {
                    if (!this.playerOwnerId || !this.playerCharacterId) return;
                    try {
                        const stats = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$clientApi$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["postJSON"])("/api/character/xp", {
                            ownerId: this.playerOwnerId,
                            characterId: this.playerCharacterId,
                            amount
                        });
                ...
                syncPlayerPosition(x, y, force = false) {
                    ...
                    const dx = Math.abs(x - this.lastSyncedX);
                    const dy = Math.abs(y - this.lastSyncedY);
                    const movedTiles = Math.max(dx, dy) / MAP_TILE_SIZE;
    ...
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])(Drawer, {
        ...
        children: [
            ...
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("ul", {
                ...
                children: list.map(([monsterId, count])=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("li", {
                        ...
                        children: [
                            ...
                            getBestiaryBadge(count) && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e8281ef7._.js.map