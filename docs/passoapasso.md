...
## 2. Estrutura de Assets (docs/assets.md)
...
- Runtime ativo: todo código PIXI fica em `core/runtime/**` e o host em `components/PixiGame.tsx`; o motor legado (incluindo `CityPhaser`, hooks e helpers em `legacy/phaser/game/**`) ficou isolado em `legacy/phaser/**` e não deve ser modificado.
...
- Toda alteração funcional deve atualizar simultaneamente `TRACKER.md` e este `docs/passoapasso.md`.
...
- NavHeader e SideMenu atualizados para consumir o ThemeProvider e aplicar o tema “kawaii” no header e na sidebar, alinhando com a paleta pastel do UniversalUi e com o módulo central de tema.
- Layout de `/play` ajustado para usar container central com aspect-[16/9] e PixiGame preenchendo 100% desse espaço, evitando altura esticada por viewport e deixando o mapa mais largo e proporcional ao HUD.
...
- Cada clique grava o ID selecionado. Controles extras permitem limpar camada, copiar JSON ou salvar diretamente no jogo (`POST /api/city/map` → `maps-data/map_city.json`).
