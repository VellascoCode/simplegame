...
- `core/runtime/**` — todo o código novo do motor PIXI (Player, NPCs, monstros, HUD, input, etc.).
  - `core/editor/**` concentra apenas as ferramentas do editor de mapas (sem lógica de jogo).
  - `core/runtime/effects` concentra efeitos compartilhados (ex.: FloatingText, partículas).
  - `core/runtime/audio.ts` guarda os helpers de áudio via WebAudio (tons rápidos para combate/XP).
- O combate do runtime respeita estritamente o grid 3×3 ao redor do jogador: slots são administrados em `components/PixiGame.tsx` e os monstros (`core/runtime/monsters/MonsterActor.ts`) só atacam/recebem dano quando ocupam um SQM adjacente (horizontal, vertical ou diagonal).
- `components/PixiGame.tsx` — host React que instancia o runtime (único ponto fora do diretório `core/`).
...
- `docs/passoapasso.md` — guia operacional que deve ser atualizado em cada ciclo.
- `TRACKER.md` — changelog vivo.
- `docs/estruturacao.md` (este arquivo) — mapa rápido da árvore de pastas e convenções.

## Convenções

1. Qualquer recurso que pertença ao runtime PIXI (sprites, HUD, AI) vive sob `core/**` ou `public/**`.
...
4. Toda alteração relevante precisa atualizar `docs/passoapasso.md` e `TRACKER.md`.
